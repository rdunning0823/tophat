...
\section{InfoBox custom title}

The InfoBox title customisation file (\verb|infoBoxTitle.locale|) can be used
to customise or localise the title displayed on the upper part of an InfoBox. 
The format is a simple text file containing \verb|<label>=<value>| pairs.  Values are text
strings delimited by double quotes, for example:
\begin{verbatim}
V GND="GS"
V TAS="TAS"
...
The \verb|<label>| is the default title used in the English language.
...
When the file is loaded at startup, only the settings present in that file
overwrite the titles defined for the configured language setting; all
other InfoBox titles are unaffected.

The file is not generated automatically it must be created or copied.

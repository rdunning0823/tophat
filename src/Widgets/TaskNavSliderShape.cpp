                      ...
                      unsigned border_width)
{
  ...
  bool draw_checkmark = (task_mode == TaskManager::MODE_ORDERED)
      && (task_size > 1)
  ...
  if (altitude_difference_valid) {
    ...
    if ((PixelScalar)width < (rc.right - rc.left - (PixelScalar)label_width -
        Layout::FastScale(15))) {
  ...
  } else if (task_mode != TaskManager::MODE_ORDERED &&
      tp.bearing_valid) {
    do_bearing = true;
    bearing = tp.delta_bearing;
  }

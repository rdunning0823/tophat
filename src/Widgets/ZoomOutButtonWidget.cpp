...
void
ZoomOutButton::OnPaint(Canvas &canvas)
{
  UPixelScalar width = (UPixelScalar)Layout::Scale(1);
  PixelRect rc = {
    PixelScalar(width), PixelScalar(width),
    PixelScalar(canvas.get_width() - width),
    PixelScalar(canvas.get_height() - width)
  };
...
ZoomOutButton &
ZoomOutButtonWidget::CreateButton(ContainerWindow &parent,
                                   ...
                                   const PixelRect &rc_map)
{
  ...
  ZoomOutButton *button =
    new ZoomOutButton(parent, dialog_look, rc_map, button_style, this, 0);

                              ...
                              const AircraftState &state_last)
{
  ...
  const int t_max_active = max(active_task_point, GetLastIntermediateAchieved() + 1);
  const int t_max = min(n_task - 1, (int)t_max_active);
...
{
  ClearMatPoints();
  GeoPoint center = (TaskSize() > 0) ? GetPoint(0).GetLocation() :

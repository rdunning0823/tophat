...
#include "MatCylinderZone.hpp"
#include "Boundary.hpp"
#include "Geo/GeoVector.hpp"

fixed
MatCylinderZone::ScoreAdjustment() const
{
  return radius;
}

GeoPoint
MatCylinderZone::GetBoundaryParametric(fixed t) const
{
...
OZBoundary
MatCylinderZone::GetBoundary() const
{
  OZBoundary boundary;
...
bool
MatCylinderZone::Equals(const ObservationZonePoint &other) const
{
  const MatCylinderZone &z = (const MatCylinderZone &)other;

  return ObservationZonePoint::Equals(other) && GetRadius() == z.GetRadius();
}

GeoPoint
MatCylinderZone::GetRandomPointInSector(const fixed mag) const
{
  GeoPoint location;

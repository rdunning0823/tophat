...
#include "org_tophat_NativeBMP085Listener.h"
...
Java_org_tophat_NativeBMP085Listener_onBMP085Values(JNIEnv *env, jobject obj,
                                                    jdouble temperature,
                                                    jint pressure)
{
...
JNIEXPORT void JNICALL
Java_org_tophat_NativeBMP085Listener_onBMP085Error(JNIEnv *env, jobject obj)
{
  jlong ptr = env->GetLongField(obj, NativeBMP085Listener::ptr_field);
  if (ptr == 0)
    return;
...
{
  cls.Find(env, "org/tophat/NativeBMP085Listener");
